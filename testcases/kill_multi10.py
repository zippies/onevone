...
class TestCase(BaseCase):
	level = 1
	desc = "学生请求结束讲解还未收到成功回调时，学生杀进程，老师是否能收到学生endexplain通知"
	caseData = {
		'students' : ['11266660160'],
		'teachers' : ['11166660160'],
	...
	def run(self):
		...
		self.endexplain(self.students,nowait=True)
		self.sleep(0.2)
		self.kill(self.students)
		self.waitCallbacks(self.teachers,'tc_onnotifystendexplain',50)

		self.stopaction(self.teachers,nowait=True)

		self.sleep(2)

		self.leaveroom(self.teachers,nowait=True)

		self.sleep(2)
		
		self.closemedia(self.teachers,nowait=True)
		
		self.sleep(60)

		self.logout(self.teachers)
		if 'tc_onnotifypeerdisconnect' in self.callbacks['teacher'][self.teachers[0]]:
			assert self.checkCallbacks(self.teachers[0],['tc_onnotifypeerdisconnect','tc_onnotifystendexplain','tc_onlogout']) is True,"receive unexpect callbacks\
			:received callbacks:%s,expected callbacks:%s" %(self.callbacks['teacher'][self.teachers[0]],['tc_onnotifypeerdisconnect','tc_onnotifystendexplain','tc_onlogout'])
		else:
			assert self.checkCallbacks(self.teachers[0],['tc_onnotifystendexplain','tc_onlogout']) is True,"receive unexpect callbacks\
			:received callbacks:%s,expected callbacks:%s" %(self.callbacks['teacher'][self.teachers[0]],['tc_onnotifystendexplain','tc_onlogout'])
