...
class TestCase(BaseCase):
	level = 1
	desc = "老师startaction后老师杀进程，学生是否能收到老师peerdisconnect和cancleorder通知"
	caseData = {
		'students' : ['11266660164'],
		'teachers' : ['11166660164'],
	...
	def run(self):
		...
		assert self.checkCallbacks(self.students[0],['st_onnotify_openstream','st_onnotifytccancelorder','st_onnotifypeerdisconnect','st_onlogout']) is True,"receive unexpect callbacks\
		:received callbacks:%s,expected callbacks:%s" %(self.callbacks['teacher'][self.students[0]],['st_onnotify_openstream','st_onnotifytccancelorder','st_onnotifypeerdisconnect','st_onlogout'])
		self.clearCallbacks()
