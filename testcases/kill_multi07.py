...
class TestCase(BaseCase):
	level = 1
	desc = "老师请求开始讲解时学生杀进程，老师是否能收到学生peerdisconnect和endexplain通知"
	caseData = {
		'students' : ['11266660157'],
		'teachers' : ['11166660157'],
	...
	def run(self):
		...
		self.beginexplain(self.teachers,multi=True)
		
		self.kill(self.students)

		self.waitCallbacks(self.teachers,'tc_onnotifypeerdisconnect',60)
		cb = self.waitCallback(self.teachers[0],'tc_onnotifystendexplain',30)[0]
		self.logger.info("code:%s,orderNo:%s,explainTime:%s,fee:%s,errorMsg:%s" %(cb.res.code,cb.res.orderNo,cb.res.explainTime,cb.res.fee,cb.res.errorMsg))
		self.logout(self.teachers)
		assert self.checkCallbacks(self.teachers[0],['tc_onnotify_openstream','tc_onnotifystendexplain','tc_onbeginexplain','tc_onnotifypeerdisconnect','tc_onlogout']) is True,"receive unexpect callbacks\
		:received callbacks:%s,expected callbacks:%s" %(self.callbacks['teacher'][self.teachers[0]],['tc_onnotify_openstream','tc_onnotifystendexplain','tc_onbeginexplain','tc_onnotifypeerdisconnect','tc_onlogout'])
		self.clearCallbacks()
