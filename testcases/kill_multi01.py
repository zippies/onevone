...
class TestCase(BaseCase):
	level = 1
	desc = "老师开始讲解后学生杀进程，老师是否能收到学生peerdisconnect和endexplain通知"
	caseData = {
		'students' : ['11266660151'],
		'teachers' : ['11166660151'],
	...
	def run(self):
		...
		self.waitCallbacks(self.teachers,'tc_onnotifypeerdisconnect',90)
		self.waitCallbacks(self.teachers,'tc_onnotifystendexplain')

		self.stopaction(self.teachers,nowait=True)
		assert self.checkCallbacks(self.teachers[0],['tc_onnotifystendexplain','tc_onnotifypeerdisconnect']) is True,"receive unexpect callbacks\
		:received callbacks:%s,expected callbacks:%s" %(self.callbacks['teacher'][self.teachers[0]],['tc_onnotifystendexplain','tc_onnotifypeerdisconnect'])
		self.clearCallbacks()
