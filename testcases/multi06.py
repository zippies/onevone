...
class TestCase(BaseCase):
	...
	desc = "老师抢单后学生取消订单"
	caseData = {
		'students' : ['11266660106'],
		'teachers' : ['11166660106'],
	...
	def run(self):
		...
		self.cancelorder(self.students)
		assert self.checkCallbacks(self.students[0],['st_oncancelorder']) is True,"receive unexpect callbacks\
		:received callbacks:%s,expected callbacks:%s" %(self.callbacks['student'][self.students[0]],['st_oncancelorder'])
		assert self.checkCallbacks(self.teachers[0],['tc_onnotifystcancelorder']) is True,"receive unexpect callbacks\
		:received callbacks:%s,expected callbacks:%s" %(self.callbacks['teacher'][self.teachers[0]],['tc_onnotifystcancelorder'])
		self.clearCallbacks()
