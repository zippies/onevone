...
class TestCase(BaseCase):
	level = 1
	desc = "老师请求抢单还未收到抢单成功回调时，老师杀进程，学生是否能收到老师peerdisconnect和cancleorder通知"
	caseData = {
		'students' : ['11266660167'],
		'teachers' : ['11166660167'],
	...
	def run(self):
		...
		assert self.checkCallbacks(self.teachers[0],['tc_onsetteacherstat'],ignore="tc_onnotifyneworder") is True,"receive unexpect callbacks\
		:received callbacks:%s,expected callbacks:%s" %(self.callbacks['teacher'][self.teachers[0]],['tc_onsetteacherstat'])
		...
		self.graborder(self.teachers,storderNo)
		...
		assert self.checkCallbacks(self.students[0],['st_onnotifygraborder','st_onnotifyorderresult','st_onnotifytccancelorder','st_onnotifypeerdisconnect','st_onlogout']) is True,"receive unexpect callbacks\
		:received callbacks:%s,expected callbacks:%s" %(self.callbacks['student'][self.students[0]],['st_onnotifygraborder','st_onnotifyorderresult','st_onnotifytccancelorder','st_onnotifypeerdisconnect','st_onlogout'])
		self.clearCallbacks()
