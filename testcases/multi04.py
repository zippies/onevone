...
class TestCase(BaseCase):
	...
	desc = "老师抢单后老师未进入房间"
	caseData = {
		'students' : ['11266660104'],
		'teachers' : ['11166660104'],
	...
	def run(self):
		...
		self.enterroomresult(self.students)

		r = self.waitCallbacks(self.students,'st_onnotifystartanalyzequestion',50)[0][0]
		assert r.res.code != 0,"abcd,code:%s,errorMsg:%s" %(r.res.code,r.res.errorMsg.decode())
		
		self.waitCallbacks(self.teachers,'tc_onnotifystcancelorder')
		self.clearCallbacks()
